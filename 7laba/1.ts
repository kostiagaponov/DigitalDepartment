namespace Transport {
	...
	const Seal = (target: Function) => {
		Object.seal(target)
		Object.seal(target.prototype)
	}

	function toUpperCaseDecorator(
		target: Object,
		propertyKey: string,
	...
	interface VehicleStorage<T extends Vehicle> {
		...
		getVehiclesByDocumentSeriesStart(seriesStart: string): T[]
	}
	...
	export class Storage<T extends Vehiclee> implements VehicleStorage<T> {
		...
		sortByOwnerSurname(): void {
			this._T.sort((a, b) => {
				const surnameA = a.informationOwner.surname.toLowerCase()
				const surnameB = b.informationOwner.surname.toLowerCase()
				return surnameA.localeCompare(surnameB)
			})
		}

		getVehiclesByDocumentSeriesStart(seriesStart: string): T[] {
			return this._T.filter(vehicle =>
				vehicle.informationOwner.series.toString().startsWith(seriesStart)
			)
		}
	}
